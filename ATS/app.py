...
def generate_summary(response_text):
    # Extract candidate summary from the response
    summary = response_text.split('"Candidate Summary":"')[1].split('"')[0]
    st.markdown('<h3 style="color: yellow; text-align: left;">Candidate Summary</h3>', unsafe_allow_html=True)
    st.write(f"<div style='text-align:left; font-family: sans-serif; font-size: 20px;'>{summary}</div>", unsafe_allow_html=True)

# Prompt Template for the ATS evaluation
...
cover_letter_prompt = """
Generate a professional cover letter for the following job description and resume:
...
col1, col2, col3, col4, col5 = st.columns([3, 3, 4, 3, 3])
...
with col3:
    submit_button2 = st.button("Find Missing Keywords")  
with col4:
    submit_button3 = st.button("Summary")  # New Button for Document Summary
with col5:
    submit_button4 = st.button("Cover Letter")  # New Button for Cover Letter

# Button Action for ATS Evaluation
if submit_button:   
    if uploaded_file is not None:
        st.lottie(lottie_checking, speed=1, loop=True, quality="low", height="200px", width="200px", key=None)
...
# Button Action for Missing Keywords
elif submit_button2:
    if uploaded_file is not None:
        st.lottie(lottie_checking, speed=1, loop=True, quality="low", height="200px", width="200px", key=None)
        if uploaded_file.type == "application/pdf":
        ...
        st.markdown('<h3 style="color: yellow; text-align: left;">Missing Keywords</h3>', unsafe_allow_html=True)
        st.write(f"<div style='text-align:left; font-family: sans-serif; font-size: 20px;'>{missing_keywords}</div>", unsafe_allow_html=True)
    else:
        st.markdown('<h6 style="color: red;text-align: center;">Please upload your Resume!</h6>', unsafe_allow_html=True)        

# Button Action for Document Summary
elif submit_button3:
    if uploaded_file is not None:
        st.lottie(lottie_checking, speed=1, loop=True, quality="low", height="200px", width="200px", key=None)
        if uploaded_file.type == "application/pdf":
        ...
        # Display Candidate Summary using generate_summary function
        generate_summary(response_text)
    else:
        st.markdown('<h6 style="color: red;text-align: center;">Please upload your Resume!</h6>', unsafe_allow_html=True)

# Button Action for Cover Letter
elif submit_button4:
    if uploaded_file is not None:
        st.lottie(lottie_checking, speed=1, loop=True, quality="low", height="200px", width="200px", key=None)
        if uploaded_file.type == "application/pdf":
            ...
            resume_text = extract_text_from_docx_file(uploaded_file)
        cover_letter = generate_response_from_gemini(cover_letter_prompt.format(
            job_description=job_description, resume_text=resume_text
        ))

        # Display Cover Letter
        st.markdown('<h3 style="color: yellow; text-align: left;">Cover Letter</h3>', unsafe_allow_html=True)
